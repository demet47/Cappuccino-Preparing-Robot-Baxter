...
X = np.array([X0, X1, X2, X3, X4]) #list of np arrays
length = len(max(X, key=len))
X = np.array([interpolateJointData(element, length) for element in X])
